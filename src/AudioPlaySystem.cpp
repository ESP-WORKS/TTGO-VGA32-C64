...
#ifdef USE_I2S
// Arduino (espressif32 6.12) roda sobre IDF 4.4, onde I2S_MODE_DAC_BUILT_IN e
// i2s_set_dac_mode ainda existem -- entao voltamos ao caminho I2S original.
// So' i2s_write_bytes saiu; usamos i2s_write.
// O VGA usa I2S1, o audio usa I2S0: nao ha conflito.
#include "esp_event.h"
...
{
#ifdef USE_I2S
	if (audioReady) return;   // construtor global nao chama mais; go.cpp chama 1x

	Buffer = (uint16_t *)malloc(DEFAULT_SAMPLESIZE * 2 * sizeof(uint16_t));
	if (!Buffer) { printf("audio: malloc failed\n"); return; }
	memset((void*)Buffer, 0, DEFAULT_SAMPLESIZE * 2 * sizeof(uint16_t));
	...
	if (i2s_driver_install(I2S_NUM, &i2s_config, 0, NULL) != ESP_OK) {
		printf("audio: i2s_driver_install falhou\n");
		return;
	}
	i2s_set_pin(I2S_NUM, NULL);
	// RIGHT_EN = DAC1 = GPIO25. LEFT_EN seria DAC2 = GPIO26, que a ponte
	// T-Display usa como TX.
...
void AudioPlaySystem::stop(void)
{
	playing = false;
#ifdef USE_I2S  	
  if (audioReady) { i2s_driver_uninstall(I2S_NUM); audioReady = false; }
#else
#endif   
}
...
void AudioPlaySystem::step(void) 
{
  ...
  while(left) {
    ...
    size_t written=0;
    i2s_write(I2S_NUM, (const void*)Buffer, n*4, &written, portMAX_DELAY);
    left-=n;
  }
#else
