...
// ---------------------------------------------------------------------------
// Fonte do pixel clock do I2S.
//   0 = PLL_D2 (160 MHz) com divisor fracionario  <-- padrao
//   1 = APLL (comportamento original do bitluni)
//
// Por que o padrao mudou: para 320x240 o calculo do APLL da VCO de
// 40 MHz x 13,846 = 553,8 MHz, fora da faixa de 350-500 MHz do TRM. O
// silicio costuma tolerar, mas o rtc_clk_apll_enable() da IDF 4.4 espera o
// bit de fim de calibracao num laco sem yield -- e com o PLL fora de faixa
// esse bit nunca vem: laco infinito e watchdog no IDLE do core.
//
// O PLL_D2 nao precisa de calibracao. Para 12,5875 MHz da N=12 a=45 b=32,
// ou seja 12,58741 MHz: 0,0007% de erro, dentro do que um monitor aceita.
// ---------------------------------------------------------------------------
#define VGA_USE_APLL 0
...
{
  printf("[i2s] periph_module_enable\n");
  ...
  // clock setup
   ...
   printf("[i2s] APLL: freq=%ld sdm=0x%05X odir=%d -> sdm0=0x%02X sdm1=0x%02X sdm2=0x%02X\n",
          freq, (unsigned)sdm, odir,
          (unsigned)(sdm & 0xff), (unsigned)((sdm >> 8) & 0xff), (unsigned)(sdm >> 16));
#if VGA_USE_APLL
   printf("[i2s] chamando rtc_clk_apll_enable...\n");
   fflush(stdout);
   rtc_clk_apll_enable(true, sdm & 0xff, (sdm >> 8) & 0xff, sdm >> 16, odir);
   printf("[i2s] rtc_clk_apll_enable VOLTOU\n");
#else
   (void)sdmn;
   printf("[i2s] APLL ignorado: usando PLL_D2\n");
#endif
  #endif

  I2S1.clkm_conf.val = 0;
#if VGA_USE_APLL
  I2S1.clkm_conf.clka_en = 1;          // fonte = APLL (programado acima)
  I2S1.clkm_conf.clkm_div_num = 2;
  I2S1.clkm_conf.clkm_div_a = 1;
  I2S1.clkm_conf.clkm_div_b = 0;
#else
  // Fonte = PLL_D2 (160 MHz). f = 160e6 / (N + b/a); queremos pixel_clock.
  {
    const double srcHz = 160000000.0;
    double ratio = srcHz / (double)pixel_clock;
    int N = (int)ratio;
    if (N < 2) N = 2;
    double frac = ratio - (double)N;
    int bestA = 1, bestB = 0;
    double bestErr = 1e18;
    for (int a = 2; a < 64; a++) {
      int b = (int)(frac * a + 0.5);
      if (b >= a) continue;
      double f = srcHz / ((double)N + (double)b / (double)a);
      double err = (f > (double)pixel_clock) ? f - pixel_clock : pixel_clock - f;
      if (err < bestErr) { bestErr = err; bestA = a; bestB = b; }
    }
    double got = srcHz / ((double)N + (double)bestB / (double)bestA);
    printf("[i2s] PLL_D2: alvo=%d Hz  N=%d a=%d b=%d  -> %.0f Hz\n",
           pixel_clock, N, bestA, bestB, got);
    I2S1.clkm_conf.clka_en = 0;
    I2S1.clkm_conf.clkm_div_num = N;
    I2S1.clkm_conf.clkm_div_a = bestA;
    I2S1.clkm_conf.clkm_div_b = bestB;
  }
#endif
  ...
  printf("[i2s] registradores configurados\n");

  i2s_isr_handle = 0;
  esp_err_t ie = esp_intr_alloc(ETS_I2S1_INTR_SOURCE, ESP_INTR_FLAG_INTRDISABLED | ESP_INTR_FLAG_LEVEL3 | ESP_INTR_FLAG_IRAM, i2s_isr, NULL, &i2s_isr_handle);
  printf("[i2s] esp_intr_alloc -> %d handle=%p\n", (int)ie, i2s_isr_handle);
}
...
static void start_i2s_output()
{
  ...
  I2S1.conf.val &= ~(I2S_RX_RESET_M | I2S_RX_FIFO_RESET_M | I2S_TX_RESET_M | I2S_TX_FIFO_RESET_M);
  {
    // Busy-wait original nao tinha saida: se o I2S1 nao sair do reset a task
    // gira para sempre e o watchdog mata o IDLE do core. Com teto viramos
    // uma mensagem em vez de um travamento mudo.
    int guard = 1000000;
    while (I2S1.state.rx_fifo_reset_back && --guard) { }
    if (!guard) printf("[vga_6bit] TIMEOUT esperando rx_fifo_reset_back\n");
  }
...
void vga_init(const unsigned char *pin_map, const int *mode, bool double_buffered)
{
  ...
  printf("[vga] modo: %dx%d (h_pixels=%d v_pixels=%d v_div=%d pixclk=%d)\n",
         x_res(), y_res(), h_pixels, v_pixels, v_div, pixel_clock);

  num_framebuffers = (double_buffered) ? 2 : 1;
  for (int i = 0; i < num_framebuffers; i++) {
    framebuffer[i] = alloc_framebuffer();
    printf("[vga] framebuffer[%d] = %p\n", i, framebuffer[i]);
    if (!framebuffer[i]) { printf("[vga] ABORTA: alloc_framebuffer falhou\n"); return; }
  }
  ...
  printf("[vga] framebuffers limpos\n");

  allocate_vga_i2s_buffers();
  printf("[vga] buffers i2s alocados (%d descritores)\n", dma_buf_desc_count);
  set_vga_i2s_active_framebuffer(framebuffer[active_framebuffer]);
  printf("[vga] descritores apontados para o framebuffer\n");

  setup_i2s_output(pin_map);  
  printf("[vga] setup_i2s_output ok\n");
  start_i2s_output();
  printf("[vga] start_i2s_output ok\n");
